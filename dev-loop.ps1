...
foreach ($item in $Items) {
  ...
  claude --dangerously-skip-permissions -p "/gds-create-story $item" --model claude-opus-4-6 --verbose

  Write-Host "`n--- Step 2: Dev Story ---" -ForegroundColor Yellow
  claude --dangerously-skip-permissions -p "/gds-dev-story $item" --model claude-opus-4-6 --verbose

  Write-Host "`n--- Step 3: Code Review ---" -ForegroundColor Yellow
  claude --dangerously-skip-permissions -p "/gds-code-review $item" --model claude-opus-4-6 --verbose
