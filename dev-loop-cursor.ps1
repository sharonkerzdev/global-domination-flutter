...
if (-not $env:CURSOR_API_KEY) {
  Write-Host "ERROR: CURSOR_API_KEY not set. In this shell run: `$env:CURSOR_API_KEY = '<your-key>'" -ForegroundColor Red
  exit 1
}
...
foreach ($item in $Items) {
  ...
  cursor-agent -p --force --output-format text --model "Gemini 3.1 Pro" "Create the story $item - follow the gds-create-story workflow" 2>&1 | Tee-Object "_bmad-output/logs/$item-step1.log"

  Write-Host "`n--- Step 2: Dev Story ---" -ForegroundColor Yellow
  cursor-agent -p --force --output-format text --model "Composer 2" "Implement the story $item - follow the gds-dev-story workflow" 2>&1 | Tee-Object "_bmad-output/logs/$item-step2.log"
  ...
  cursor-agent -p --force --output-format text --model "Codex 5.3" "Run a code review on the latest changes for story $item - follow the gds-code-review workflow" 2>&1 | Tee-Object "_bmad-output/logs/$item-step3.log"
