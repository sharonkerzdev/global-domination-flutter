...
- [ ] Task 1: Add `UnlockCountry({required CountryId countryId})` to `lib/game/game_command.dart` (AC: #1, #3, #4, #5, #6)
  - [ ] Subtask 1.1: Mirror the shape of `HireLeader` / `TapCountry` (sealed `final class`, manual `==` / `hashCode` / `toString`).
  - [ ] Subtask 1.2: Update `test/game/game_command_test.dart` with equality + `toString` coverage.
- [ ] Task 2: Add `CountryUnlocked` to `lib/game/game_event.dart` (AC: #1)
  - [ ] Subtask 2.1: Fields `{CountryId countryId, ContinentId continent, Influence cost}` plus inherited `at`. Manual `==` / `hashCode` / `toString`.
  - [ ] Subtask 2.2: Update `test/game/game_event_test.dart`.
- [ ] Task 3: Create `lib/game/features/continents/unlocks_reducer.dart` with `applyUnlockCountry(state, content, cmd, {required DateTime now})` (AC: #1, #3, #4, #5, #6)
  - [ ] Subtask 3.1: Validate command shape (country exists in `state.countries` AND `content.countries`) → `MissingCountry`.
  - [ ] Subtask 3.2: Already-unlocked guard → `Locked(reason: 'already_unlocked')`.
  - [ ] Subtask 3.3: Continent-unlocked guard via `state.totalInfluence >= continentDef.unlockThreshold` → `Locked(reason: 'continent_locked')`. If `continentDef` is missing from content → `MissingCountry` (use the country's missing-from-content path, do NOT invent a new error variant).
  - [ ] Subtask 3.4: Affordability check via `def.unlockCost` → `InsufficientFunds(required: Influence(def.unlockCost))`.
  - [ ] Subtask 3.5: Build new `CountryState` with `unlocked: true, ipLevel: 1, leaderTier: LeaderTier.none, bankedInfluence: Influence.zero, lastCollectedAt: null` and new `GameState` with deducted `totalInfluence`. Emit `CountryUnlocked(now, countryId, def.continent, Influence(def.unlockCost))`.
- [ ] Task 4: Wire `UnlockCountry` into `GameWorld.applyCommand` switch in `lib/game/game_world.dart` (AC: #1)
  - [ ] Subtask 4.1: Add `_applyUnlockCountry(cmd)` helper following the `_applyHireLeader` template — adopt the same `result.map((tuple) { ... })` pattern, push state, emit event.
- [ ] Task 5: Pure-Dart unit tests in `test/game/features/continents/unlocks_reducer_test.dart` (AC: #1, #3, #4, #5, #6)
  - [ ] Subtask 5.1: Happy path (unlock nigeria from africa with enough Influence) — assert state mutation, event payload, and that totalInfluence decreased exactly by `def.unlockCost`.
  - [ ] Subtask 5.2: Already-unlocked → `Locked('already_unlocked')`, no mutation.
  - [ ] Subtask 5.3: Continent locked (use a test continent with `unlockThreshold > totalInfluence`) → `Locked('continent_locked')`, no mutation.
  - [ ] Subtask 5.4: Insufficient funds → `InsufficientFunds(required: Influence(def.unlockCost))`, no mutation.
  - [ ] Subtask 5.5: Missing country (id not in content) → `MissingCountry(id: ...)`.
  - [ ] Subtask 5.6: Africa edge: `unlockThreshold = 0` means continent always unlocked; nigeria with `unlockCost = 5` and `totalInfluence = Influence(Decimal.fromInt(5))` succeeds (boundary `>=`).
- [ ] Task 6: Integration test in `test/game/game_world_test.dart` (AC: #1, #8)
  - [ ] Subtask 6.1: Dispatch `UnlockCountry` through `GameWorld.applyCommand`, observe state change AND emitted event on `world.events` stream.
  - [ ] Subtask 6.2: After unlock, call `world.tick(Duration(seconds: 1))` and assert the newly unlocked country's `bankedInfluence` increased (proves AC #8 — no special-casing needed).
- [ ] Task 7: Verify `CountryDef.unlockCost` is consumed correctly (AC: #2)
  - [ ] Subtask 7.1: Confirm no NEW computation of "previous × 5" appears anywhere in `lib/game/`. Cost MUST come from `def.unlockCost`. Add a comment in the reducer if needed.
  - [ ] Subtask 7.2: Optional: extend `test/architecture/no_duplicate_income_math_test.dart` is NOT in scope here (that test is income-specific); do not modify it.
